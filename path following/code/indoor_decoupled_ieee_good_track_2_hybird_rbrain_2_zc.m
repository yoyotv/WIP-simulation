...
for i=1:2
    delta_weight_a(i,:)=-0.0001*input(i).*s(i,:);%(error converge much quickly as learning rate minuslearning)
     delta_weight_p(i,:)=0.00001*input(i).*s(i,:);
    
   delta_mean(i,:)=-(0.1*input(i))*[(weight_a(i,:)-weight_p(i,:))].*[s(i,:)].*(2*(inputmatrix(i)-mean(i,:))./(var(i,:).^2));
   delta_var(i,:)=-(0.1*input(i))*[(weight_a(i,:)-weight_p(i,:))].*[s(i,:)].*(2*(inputmatrix(i)-mean(i,:)).^2./(var(i,:).^3));
    ...
    ub(1)=s(1,:)*weight_a(1,:)'-1*s(1,:)*weight_p(1,:)';
        ub(2)=s(2,:)*weight_a(2,:)'-1*s(2,:)*weight_p(2,:)';
    %===============================================
...
% tu_w=(-c1*error_w-d_wd+f_1-10*sat(sliding(1))-1*ub(1))/(-g1);
% tu_v=(-c3*d_theta+c3*d_z-f_3-50*sat(sliding(2))-1*ub(2))/(g3);

tu_w=(-c1*error_w-d_wd+f_1-20*sat(sliding(1)))/(-g1)-1*0.01*ub(1);
tu_v=(-c3*d_theta+c3*d_z-f_3-30*sat(sliding(2)))/(g3)-1*0.001*ub(2);
