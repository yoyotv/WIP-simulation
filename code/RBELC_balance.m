...
inputmatrix=zeros(2,10);
    weight_a=zeros(2,10)*0.1;
    weight_p=zeros(2,10)*0.1;
...
for i=1:2
    delta_weight_a(i,:)=-0.02*sliding(i).*s(i,:);%(error converge much quickly as learning rate minuslearning)
    delta_weight_p(i,:)=0.01*sliding(i).*s(i,:);
    
           delta_rw(i,:)=-(0.5*sliding(i))*[(weight_a(i,:)-weight_p(i,:))].*[s(i,:)].*(2*(inputmatrix(i)-mean(i,:))./(var(i,:).^2)).*p_s(i,:);
   
    
    
   delta_mean(i,:)=-(3*sliding(i))*[(weight_a(i,:)-weight_p(i,:))].*[s(i,:)].*(2*(inputmatrix(i)-mean(i,:))./(var(i,:).^2));
   delta_var(i,:)=-(3*sliding(i))*[(weight_a(i,:)-weight_p(i,:))].*[s(i,:)].*(2*(inputmatrix(i)-mean(i,:)).^2./(var(i,:).^3));
    ...
    inputmatrix(1,:)=ones(1,value)*sliding(1);
    inputmatrix(2,:)=ones(1,value)*sliding(2);

    %sensory_cortex layer===========
    for j=1:2
    for i=1:value
    s(j,i)=exp(-((sliding(j)-mean(j,i))^2)*(var(j,i)^-2));
    ...
    ub(1)=s(1,:)*weight_a(1,:)'-s(1,:)*weight_p(1,:)';
        ub(2)=s(2,:)*weight_a(2,:)'-s(1,:)*weight_p(2,:)';

    %=====================================================================
    %===============================================
    %organize input of WIP system
    tu_w=ub(1)+0*0.1*sign(sliding(1));
    tu_v=ub(2)+0*0.1*sign(sliding(1));
    %===============================================
    %WIP system model
